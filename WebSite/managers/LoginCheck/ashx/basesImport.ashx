<%@ WebHandler Language="C#" Class="basesImport" %>
...
public class basesImport : IHttpHandler
{
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (dt.Rows[i]["用户名"].ToString() != string.Empty)
                {
                    ...
                    {
                        if (dt.Rows[i]["专业基地名称"].ToString() == dtPD.Rows[k]["professional_base_name"].ToString())
                        {
                            t2 = k;
                        }
                    }
                    string sql = "insert into GP_Login (id,name,password,real_name,training_base_name,training_base_code,professional_base_name,professional_base_code,type,LoginState,RegisterDate) values(" +
                ...
                ",'" + dtPD.Rows[t2]["professional_base_code"].ToString() + "','bases','1','" + DateTime.Now.Year.ToString() + "')";
            ...
            if (rows > 0)
            {
                context.Response.Write("专业基地负责人基本信息导入成功");
            }
            else
            {
                context.Response.Write("专业基地负责人基本信息导入失败");
            }
        }
